...
tests.runExpressionTests = function(){
	...
	test( "test YASMIJ.Expression.convertExpressionToObject() with valid input", function(){
		...
		deepEqual( fn( "a + 20b + 10" ), {"1":10,"b":20,"a":1} );
		deepEqual( fn( "a + b -c + 4e" ), {"e":4,"c":-1,"b":1,"a":1} );
		deepEqual( fn( "-a - 1b -20c - 1.23e43d -1e-13e" ), { "e":-1e-13, "d":-1.23e+43,"c":-20,"b":-1,"a":-1} );
		deepEqual( fn( "+a + 1b +20c + 1.23e43d +1e-13e" ), { "e":1e-13, "d":1.23e+43,"c":20,"b":1,"a":1} );
	});
	test( "test YASMIJ.Expression.convertExpressionToObject() for single number", function(){
		...
		deepEqual(fn( "" ), {1:0} );
		deepEqual(fn( "1" ), {1:1} );
		deepEqual(fn( "-20" ), {1:-20} );
		deepEqual(fn( "-423.12345" ), {1:-423.12345} );
		deepEqual(fn( "-1.23e-34" ), {1:-1.23e-34} );
	});
	test( "test YASMIJ.Expression.convertExpressionToObject() for single variables", function(){
		...
		deepEqual(fn( "-1x" ), {x:-1} );
		deepEqual(fn( "-2x" ), {x:-2} );
		deepEqual(fn( "-20.2x" ), {x:-20.2} );
		deepEqual(fn( "-20.000042x" ), {x:-20.000042} );
		deepEqual(fn( "-20e34x" ), {x:-20e34} );
		deepEqual(fn( "-1.23e-34x" ), {x:-1.23e-34} );
	});
	
	test( "test YASMIJ.Expression.convertExpressionToObject() for numbers and variables", function(){
		var fn = YASMIJ.Expression.convertExpressionToObject;
		deepEqual(fn( "1 + x" ), {x:1, 1:1} );
		deepEqual(fn( "x + 1" ), {x:1, 1:1} );
		deepEqual(fn( "-x - 1" ), {x:-1, 1:-1} );
		deepEqual(fn( "-1 -x" ), {x:-1, 1:-1} );
		deepEqual(fn( "-20.2x + 2.4y +10e30" ), {x:-20.2, y:2.4, 1:10e30} );
		deepEqual(fn( "-20e34x + 23 - 4 - 4x + 1" ), {x:-2e+35, 1:20} );
	});	
	...
	test("test YASMIJ.Expression.parse() with valid YASMIJ.Expressions", function(){
		...
		deepEqual( fn( "10x1 -2x2 - 10" ), {"x1":10,"x2":-2, "1":-10} );
	});
