...
tests.runMatrixTests = function(){
    ...
    test( "test YASMIJ.Matrix.getMaxArray()", function(){
        ...
        equal( fn(), null );
        deepEqual( fn( [[]] ), {index:0, max:0} );
        deepEqual( fn( [1,2,3] ), {index:0, max:3} );
        deepEqual( fn( [[],[]] ), {index:0, max:0} );
        deepEqual( fn( [[1],[]] ), {index:0, max:1} );
        deepEqual( fn( [[1],[1,2]] ), {index:1, max:2} );
        deepEqual( fn( [[1],[1,2],[1,2,3]] ), {index:2, max:3} );
    });
